...
(function ($) {
	...
	var Ideabit = {
		init: function () {
			Ideabit.debouncedAutoPlayPauseVideos();
			var debouncedAutoPlayPauseVideos = Ideabit.debounce( Ideabit.debouncedAutoPlayPauseVideos, 300 );
			$( window ).scroll( debouncedAutoPlayPauseVideos );
			Ideabit.handleFreemiusPayment();
		...
		handleFreemiusPayment: function () {
			var fs_args = {
				...
				image: 'https://ideabit.co/assets/img/mcg-fs.jpg',
			};
			...
			var handler = FS.Checkout.configure( fs_args );

			
			$('.ib-buy-btn').click( function (e) {
				console.log( 'ideabit: checkout triggered' );
				handler.open( {
					name     : 'Multi-Currency for GravityForms',
					licenses : $(this).data('sites'),
					// You can consume the response for after purchase logic.
					purchaseCompleted: function (response) {
						// The logic here will be executed immediately after the purchase confirmation.
						// alert(response.user.email);
						Ideabit.start_confetti()
					},
		...
		debouncedAutoPlayPauseVideos: function () {
			$( '.play-when-in-view' ).each( function () {
				if ( Ideabit.checkVisible( $( this ) ) ) {
					this.play();
				} else {
					this.pause();
				}
	...
	$(window).on('load', Ideabit.init);

	// window.Ideabit = Ideabit
} )( jQuery );
